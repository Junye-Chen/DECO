...
from core.dataset_CPN import TrainDataset
...
class Trainer:
    def __init__(self, config):
        ...
        # ------- IBIP Network ------------------------------------------------------
        self.netG = Refinement(in_channels=4, out_channels=3, n_skips=0).to(self.config['device'])
        self.netG.train()
        # ----------------------------------------------------------------------------------------------
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            # wm, clean, mask, gt = wm.to(device), clean.to(device), mask.to(device).float(), gt.to(device)
            wm, mask, clean = wm.to(device), mask.to(device).float(), gt.to(device)

            pred, _ = self.netG(wm, mask)

            gen_loss = 0
            dis_loss = 0
            self.optimG.zero_grad()
            # print(pred.shape, masks.shape, clean.shape)

            # generator l1 loss
            hole_loss = self.l1_loss(pred * masks, clean * masks)
            ...
            valid_loss = self.l1_loss(pred * (1 - masks), clean * (1 - masks))
            ...
            # perceptual loss
            if self.config['losses']['perceptual_weight'] > 0:
                perc_loss = self.perc_loss(pred.view(-1, 3, h, w), clean.view(-1, 3, h, w))[0] * self.config['losses']['perceptual_weight']
                gen_loss += perc_loss
                self.add_summary(self.gen_writer, 'loss/perc_loss', perc_loss.item())

            # self-perceptual loss
            if self.config['losses']['selfperceptual_weight'] > 0 and self.iteration > 5:  # 200000
                _, pred_enc_feat, pred_x2 = self.netG.module.encoder(
                    torch.cat([pred_imgs.view(b * t, -1, h, w), updated_masks.view(b * t, -1, h, w)], dim=1))
                _, enc_feat, x2 = self.netG.module.encoder(torch.cat([frames.view(b * t, -1, h, w), updated_masks.view(b * t, -1, h, w)], dim=1))
                sp_loss = self.sploss((pred_enc_feat, pred_x2), (enc_feat.detach(), x2.detach()))
                gen_loss += sp_loss
                self.add_summary(self.gen_writer, 'loss/self_perc_loss', sp_loss.item())
            ...
            if self.iteration % 1000 == 0:
                # img to cpu
                t = 0
                clean_cpu = ((clean.view(b, -1, 3, h, w) + 1) / 2.0).cpu()
                wm_cpu = ((wm.view(b, -1, 3, h, w) + 1) / 2.0).cpu()
                pred_cpu = ((pred.view(b, -1, 3, h, w) + 1) / 2.0).cpu()
                img_results = torch.cat([wm_cpu[0][t], clean_cpu[0][t], pred_cpu[0][t]], 1)
                ...
                psnr, ssim = 0, 0
                # print(clean_cpu.shape)
                _, t, _, _, _ = clean_cpu.shape
                for i in range(t):
                    psnr += self.psnr(pred_cpu[0][i].unsqueeze(0), clean_cpu[0][i].unsqueeze(0))
                    ssim += self.ssim(pred_cpu[0][i].unsqueeze(0), clean_cpu[0][i].unsqueeze(0))
                psnr /= t
                ssim /= t
