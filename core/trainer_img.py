...
from core.loss import AdversarialLoss, PerceptualLoss, LPIPSLoss, make_r1_gp, SelfPerceptualLoss, StyleLoss
...
class Trainer:
    def __init__(self, config):
        ...
        self.netG = ImageGeneratorVQ(config['model']).to(self.config['device'])
        self.netG.train()
        ...
        if not self.config['model'].get('no_dis', False):
            ...
            self.netD.load_state_dict(torch.load("experiments_model/image_train_img/weights/dis_200000.pth"))
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            with torch.no_grad():
                clean, _ = self.cpn(wm, mask)
            fine_frame, output_inp, output_res, l_codebook, l_trans,_ = self.netG(clean, mask, gt)
            n, c, h, w = fine_frame.shape
            ...
            # fusion branch
            f_loss = 0
            hole_loss = self.l1_loss(fine_frame * mask, gt * mask)
            hole_loss = hole_loss / (torch.mean(mask) + 1e-6)
            f_loss += hole_loss
            valid_loss = self.l1_loss(fine_frame * (1 - mask), gt * (1 - mask))
            valid_loss = valid_loss / (torch.mean(1 - mask) + 1e-6)
            f_loss += valid_loss
            self.add_summary(self.gen_writer, 'loss/fusion', f_loss.item())
            # --------------------------------------------------

            i_loss = 0
            # codebook loss
            l_codebook *= self.config['losses']['codebook_weight']
            i_loss += l_codebook.mean()

            # semantic cluster loss
            l_trans *= self.config['losses']['index_weight']
            i_loss += l_trans.mean()

            # Reconstruction Loss
            hole_loss = self.l1_loss(output_inp * mask, gt * mask)
            hole_loss = hole_loss / (torch.mean(mask) + 1e-6)
            i_loss += hole_loss
            valid_loss = self.l1_loss(output_inp * (1 - mask), gt * (1 - mask))
            valid_loss = valid_loss / (torch.mean(1 - mask) + 1e-6)
            i_loss += valid_loss

            # generator adversarial loss
            gen_clip = self.netD(output_inp.view(n, 1, c, h, w))
            gan_loss = self.adversarial_loss(gen_clip, True, False)
            gan_loss = gan_loss * self.config['losses']['adversarial_weight']
            i_loss += gan_loss
            self.add_summary(self.gen_writer, 'loss/VQ', i_loss.item())
            # --------------------------------------------------

            # restoration branch
            r_loss = 0
            hole_loss = self.l1_loss(output_res * mask, gt * mask)
            hole_loss = hole_loss / (torch.mean(mask) + 1e-6)
            r_loss += hole_loss
            valid_loss = self.l1_loss(output_res * (1 - mask), gt * (1 - mask))
            valid_loss = valid_loss / (torch.mean(1 - mask) + 1e-6)
            r_loss += valid_loss
            perc_loss = self.perc_loss(output_res.view(-1, 3, h, w), gt.view(-1, 3, h, w))[0] * self.config['losses']['perceptual_weight']
            r_loss += perc_loss
            # gram_loss = self.gramloss()
            self.add_summary(self.gen_writer, 'loss/restoration', r_loss.item())
            # --------------------------------------------------

            gen_loss = gen_loss + f_loss + i_loss + r_loss
            gen_loss.backward()
            self.optimG.step()
            ...
            if not self.config['model']['no_dis']:
                ...
                fake_clip = self.netD(output_inp.detach().view(n, 1, c, h, w))
            ...
            if self.iteration % 1000 == 0:
                ...
                clean_cpu = ((clean.view(n, -1, 3, h, w) + 1) / 2.0).cpu()
                gt_cpu = ((gt.view(n, -1, 3, h, w) + 1) / 2.0).cpu()
                mask_cpu = (mask.view(n, -1, 1, h, w)).cpu()
                fine_frame_cpu = ((fine_frame.view(n, -1, 3, h, w) + 1) / 2.0).cpu()
                output_inp_cpu = ((output_inp.view(n, -1, 3, h, w) + 1) / 2.0).cpu()
                output_res_cpu = ((output_res.view(n, -1, 3, h, w) + 1) / 2.0).cpu()
                img_results = torch.cat([wm_cpu[0][t], clean_cpu[0][t], fine_frame_cpu[0][t],
                                         output_inp_cpu[0][t], output_res_cpu[0][t], gt_cpu[0][t]], 1)
                ...
                if self.gen_writer is not None:
                    ...
                    self.gen_writer.add_image(f'img/mask:{t}', mask_cpu[0][t], self.iteration)

                # metric
                fpsnr, fssim = self.metrics(fine_frame_cpu[0][t], gt_cpu[0][t])
                ipsnr, issim = self.metrics(output_inp_cpu[0][t], gt_cpu[0][t])
                rpsnr, rssim = self.metrics(output_res_cpu[0][t], gt_cpu[0][t])
                print(f'fine: PSNR: {fpsnr:.2f}', " ", f'SSIM: {fssim:.4f}')
                print(f'inpt: PSNR: {ipsnr:.2f}', " ", f'SSIM: {issim:.4f}')
                print(f'rest: PSNR: {rpsnr:.2f}', " ", f'SSIM: {rssim:.4f}')
            ...
            if self.config['global_rank'] == 0:
                pbar.update(1)
                pbar.set_description((f"fine: {f_loss.item():.3f}; "
                                      f"inpt: {i_loss.item():.3f} (l1: {(hole_loss.item() + perc_loss.item()):.3f}, "
                                      f"VQ: {l_codebook.item():.3f}, trans: {l_trans.item():.3f}, gan: {gan_loss.item():.3f}); "
                                      f"rest: {r_loss.item():.3f}; "
                                      f"dis: {dis_loss.item():.3f}"))

                if self.iteration % self.train_args['log_freq'] == 0:
                    logging.info(f"[Iter {self.iteration}] "
                                 f"fusion: {f_loss.item():.4f}; "
                                 f"inpainting: {i_loss.item():.4f}; "
                                 f"restoration: {r_loss.item():.4f}")
