...
from model.image_model import FeatureInjectionGate
...
def fbConsistencyCheck(flow_fw, flow_bw, alpha1=0.01, alpha2=0.5):
    flow_bw_warped = flow_warp(flow_bw, flow_fw.permute(0, 2, 3, 1).contiguous())  # wb(wf(x))
...
class BidirectionalPropagation(nn.Module):
    ...
    def forward(self, x, flows_forward, flows_backward, mask, interpolation='bilinear'):
        ...
        b, t, c, h, w = x.shape
        feats, masks = {}, {}
        feats['input'] = [x[:, i, :, :, :] for i in range(0, t)]  # 把 x 从 tensor 拆成 frame list
        ...
        for p_i, module_name in enumerate(prop_list):
            ...
            for i, idx in enumerate(frame_idx):
                ...
                if i == 0:
                    ...
                    feat_warped = flow_warp(feat_prop, flow_prop.permute(0, 2, 3, 1).contiguous(), interpolation)
                    ...
                    if self.learnable:
                        ...
                        mask_prop_valid = flow_warp(mask_prop, flow_prop.permute(0, 2, 3, 1).contiguous())
        ...
        return outputs_b.reshape(b, -1, c, h, w), outputs_f.reshape(b, -1, c, h, w), outputs.reshape(b, -1, c, h, w), masks_f
...
class InpaintGenerator2(BaseNetwork):
    """
        propainter without flow guidance
    """
    ...
    def __init__(self, init_weights=True, model_path=None):
        ...
        self.gate = FeatureInjectionGate(in_ch=256, out_ch=channel)
        ...
        if model_path is not None:
            ...
            print('using Propainter w/o FLOW!')
    ...
    def forward(self, masked_frames, masks, num_local_frames, wm_feat_local=None, img_feat=None, t_dilation=2):

        l_t = num_local_frames
        b, t, _, ori_h, ori_w = masked_frames.size()
        local_masks = masks[:, :l_t, ...].reshape(b * l_t, 1, ori_h, ori_w)

        enc_feat = self.encoder(torch.cat([masked_frames.view(b * t, 3, ori_h, ori_w),
                                           masks.view(b * t, 1, ori_h, ori_w)], dim=1))
        ...
        if img_feat is not None:
            inj_feat = self.gate(wm_feat_local, local_masks, img_feat)
        enc_feat = enc_feat.view(b, t, c, h, w)
        updated_enc_feat = enc_feat.clone()
        enc_feat_local = enc_feat[:, :l_t, ...].reshape(b * l_t, c, h, w)
        enc_feat_local = enc_feat_local + inj_feat if inj_feat is not None else enc_feat_local
        updated_enc_feat[:, :l_t, ...] = enc_feat_local.reshape(b, l_t, c, h, w)

        ds_mask_in = F.interpolate(masks.reshape(-1, 1, ori_h, ori_w), scale_factor=1 / 4, mode='nearest').view(b, t, 1, h, w)
        ...
        trans_feat = self.ss(updated_enc_feat.view(-1, c, h, w), b, fold_feat_size)
        ...
        updated_enc_feat = updated_enc_feat + trans_feat

        if self.training:
            output = self.decoder(updated_enc_feat.view(-1, c, h, w))
            output = torch.tanh(output).view(b, t, 3, ori_h, ori_w)
        else:
            output = self.decoder(updated_enc_feat[:, :l_t].view(-1, c, h, w))
